...
export function EstoqueSocarraoTable({ veiculos }: { veiculos: VeiculoRow[] }) {
  // só portais ativos por enquanto; UI já é multi-portal
  ...
  // seleção = veículos marcados para sincronizar (já começa com os sincronizados marcados)
  const [selected, setSelected] = useState<Set<string>>(() => new Set(syncedIds));

  const formId = "portal-sync-form";
  const novos = [...selected].filter((id) => !syncedIds.has(id)); // marcados que ainda não estão no portal

  function toggle(v: VeiculoRow) {
    const isSel = selected.has(v.id);
    if (!isSel) {
      const ok = window.confirm(`Sincronizar "${v.marca} ${v.modelo}" com o ${portal?.label}?`);
      if (!ok) return;
    }
    setSelected((cur) => {
  ...
  return (
    <>
      <form id={formId} action={syncSocarraoSelecionados}>
        ...
        {[...selected].map((id) => (
          <input key={id} type="hidden" name="veiculo_id" value={id} />
        ))}
      </form>

      {/* barra de portais */}
      <div className="mt-6 flex flex-wrap items-center justify-between gap-3 border border-white/10 bg-[var(--color-panel)] p-3">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {PORTAIS.map((p) => (
            <button
              ...
              type="button"
              disabled={!p.ativo}
              onClick={() => p.ativo && trocarPortal(p.key)}
              className={`px-3 py-1.5 text-xs font-black uppercase transition-colors ${
                p.key === portalKey ? "bg-[var(--color-red)] text-white"
                : p.ativo ? "border border-white/15 text-white hover:border-[var(--color-red)]"
        ...
        <div className="flex items-center gap-3">
          <span className="text-xs text-[var(--color-mute)]">
            {syncedIds.size} sincronizado(s) · {novos.length} novo(s) selecionado(s)
          </span>
          <button
            type="submit"
            form={formId}
                ...
                <tr key={v.id} className={`border-t border-white/10 hover:bg-white/[0.03] ${v.status === "inativo" ? "opacity-50" : ""}`}>
                  <td className="px-4 py-3">
                    <label className="flex cursor-pointer items-center gap-2">
                      ...
                      {sincronizado ? (
                        <span className="text-[10px] font-black uppercase text-emerald-400">✓ no ar</span>
                      ) : marcado ? (
                        <span className="text-[10px] font-black uppercase text-amber-300">novo</span>
                      ) : null}
                    </label>
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3">
                    ...
                    <p className="text-xs text-[var(--color-mute)]">{v.versao}</p>
